...
def load_world_data(conn, countries_list):
    ...
    years = [2018, 2019]
    sql = "INSERT INTO World_Emissions (record_id, impact_country, raw_material, valued_result, tier, business_unit, year, raw_material_group, process_step, environmental_impact_group) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    cursor = conn.cursor()
    ...
    for current_year in years:
        for current_country in countries_list:
            ...
            conn.commit()
            print("Submitted " + str(len(insert_list)) + " for {country} in {year}".format(country = current_country, year = current_year))
